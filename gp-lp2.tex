...
\begin{definition}{Mixed Observable-Non-Observable Markov Decision Process (MONOMDP) }\label{def:mdpp}
  % Relate to MDPs/POMDPs?
  % We're avoiding explicit state enumeration and still
  % getting exact answers, maybe use 'symbolic' / 'bounded'.
  % Relate some kind of markov process to these constraints.
  We describe a \emph{Mixed Observable-Non-Observable Markov Decision
    Process} (MONOMDP) as a transition system composed of a mix of
  random variables (representing state components which we cannot know
  the true value of during the solution of the problem -- that is,
  non-observable) and binary variables (which are fully observable),
  and an objective function we want to maximize while obeying the
  transition system. A MONOMDP $M = (X, f, g)$ is:

  \begin{itemize}
  \item A set of $m$ binary decision variables $B = \{b_{1}...b_{m}\}, b_i \in \{0,1\} \forall 0 \leq i \leq m$

  \item A set of $n$ continuous random variables $R = \{r_{1}...r_{n}\}$.

  \item The state space $X = (B, R)$. A state $x \in X$ composed of
    $M$ binary variables and $N$ random variables, that is,
    $x = (B,R)$. For convenience, we'll index state variables in $x$
    from $0$ to $N+M$. That is $v_{1}$ would be the first binary
    variable (unless $B = \emptyset$), and $v_{M}$ would be the first
    random variable.

  \item Some transition function $f : X \rightarrow X$. Denoting a
    state variable $v$ in the 'next step' as $v_{k+1}$, this
    transition function must take the form:
    ...
    That is, each random variable must be defined as a posynomial
    function of decision variables from the previous step.

  \item $g(X) \rightarrow \mathbb{R}$, a utility function, which takes the form:
...
list only the $\text{true}$ variable, the
$P(r_{i,(k+1)}=\text{false})$ is symmetrical:

\begin{equation}
  P(r_{i,(k+1)}\alpha \leq \text{true}) > \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} \forall 0 \leq i \leq M, \alpha > 0
  P(r_{i,(k+1)}\beta \leq \text{true}) < \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} \forall 0 \leq i \leq M, 0 < \beta < 1
\end{equation}

Then the first equation easily takes the desired form:
\begin{equation}
  1 > \frac{(\sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} \forall 0 \leq i \leq M)}{P(r_{i,(k+1)}) \alpha}, \alpha > 0
\end{equation}

The second requires more algebra and also leverages the fact that in
our problem definition, we require separate equations for both the
true and false probabilities as separate decision variables:

\begin{align*}
  P(r_{i,(k+1)} \text{true})\beta &< \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} \forall 0 \leq i \leq M \\
  0 &< \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} - P(r_{i,(k+1)} = true)\beta \\
  0 &< \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} - (1 - P(r_{i,(k+1)} = false))\beta \\
  0 &< \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} - 1\beta + P(r_{i,(k+1)} = false)\beta \\
  0 &< \sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}} - 1\beta + P(r_{i,(k+1)} = false)\beta  \\
  1 &< (\sum_{j} c^{\top}_j \prod_{l} v_{l,k}^{a_{j,l}})/\beta + P(r_{i,(k+1)} = false)
\end{align*}
